...
// 2.get_temporary_buffer/release_temporary_buffer：获取/释放临时缓冲区
template <class T>
pair<T*, ptrdiff_t> get_buffer_helper(ptrdiff_t len, T*){
    ...
    while (len > 0){
        ...
        //当系统内存资源有限或者碎片化严重时，较小的内存分配请求更有可能成功
    }
...
// 3.temporary_buffer：进行临时缓冲区的申请与释放
...
temporary_buffer<ForwardIterator, T>::temporary_buffer(ForwardIterator first, ForwardIterator last){
    try{
        len = mystl::distance(first, last); //计算迭代器距离
        allocate_buffer();
        if (len > 0){
...
public:
    ...
    //拷贝构造
    auto_ptr(auto_ptr& rhs) :m_ptr(rhs.release()) {} //将原对象的资源指针置为空
    // 可以从其他类型的auto_ptr对象中获取资源
    template <class U>
